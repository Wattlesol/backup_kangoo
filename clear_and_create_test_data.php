...
echo "🧹 Clearing existing test data and creating fresh data\n";
echo "=" . str_repeat("=", 55) . "\n\n";
...
Store::withTrashed()->forceDelete();
ProductCategory::withTrashed()->forceDelete();
\DB::statement('SET FOREIGN_KEY_CHECKS=1;');
echo "  ✅ Cleared all existing e-commerce data\n\n";
...
echo "📋 Step 1: Creating Product Categories (4 total, will delete 1)\n";
...
echo "\n📦 Step 2: Creating Products (4 per category, will delete 1)\n";
...
foreach ($createdCategories as $category) {
    ...
    $products = [
        [
            ...
            'slug' => strtolower(str_replace(' ', '-', $category->name === 'Electronics' ? 'iPhone 15 Pro' :
                     ($category->name === 'Home & Garden' ? 'Ergonomic Office Chair' :
                     ($category->name === 'Fashion & Clothing' ? 'Designer Leather Jacket' : 'Professional Tennis Racket')))) . '-' . time(),
            'sku' => 'SKU-' . strtoupper(substr($category->name, 0, 3)) . '-' . rand(1000, 9999),
        ...
        [
            ...
            'slug' => strtolower(str_replace(' ', '-', $category->name === 'Electronics' ? 'MacBook Pro M3' :
                     ($category->name === 'Home & Garden' ? 'Smart LED Light Bulbs' :
                     ($category->name === 'Fashion & Clothing' ? 'Premium Running Shoes' : 'Yoga Mat Set')))) . '-' . time() . '-2',
            'sku' => 'SKU-' . strtoupper(substr($category->name, 0, 3)) . '-' . rand(1000, 9999),
            'description' => 'High-quality product designed for performance and reliability. Comes with warranty and customer support.',
            'base_price' => rand(150, 1500),
            'product_category_id' => $category->id,
            'status' => 1,
            'is_featured' => 0,
        ...
        [
            ...
            'slug' => strtolower(str_replace(' ', '-', $category->name === 'Electronics' ? 'Wireless Headphones' :
                     ($category->name === 'Home & Garden' ? 'Kitchen Knife Set' :
                     ($category->name === 'Fashion & Clothing' ? 'Casual Cotton T-Shirt' : 'Fitness Tracker Watch')))) . '-' . time() . '-3',
            'sku' => 'SKU-' . strtoupper(substr($category->name, 0, 3)) . '-' . rand(1000, 9999),
        ...
        [
            ...
            'slug' => strtolower(str_replace(' ', '-', $category->name === 'Electronics' ? 'Gaming Mouse' :
                     ($category->name === 'Home & Garden' ? 'Indoor Plant Pot' :
                     ($category->name === 'Fashion & Clothing' ? 'Winter Wool Scarf' : 'Resistance Bands Set')))) . '-' . time() . '-4',
            'sku' => 'SKU-' . strtoupper(substr($category->name, 0, 3)) . '-' . rand(1000, 9999),
            'description' => 'Specialized product with unique features. Perfect for enthusiasts and professionals alike.',
            'base_price' => rand(25, 250),
            'product_category_id' => $category->id,
            'status' => 1,
            'is_featured' => 0,
    ...
    foreach ($products as $productData) {
        ...
        echo "    ✅ Created product: {$product->name} (ID: {$product->id}) - $" . $product->base_price . "\n";
    }
}

echo "\n🏪 Step 3: Creating Stores (4 total, will delete 1)\n";
...
echo "\n🛒 Step 4: Creating Sample Orders (3 total)\n";
...
$orders = [
    [
        ...
        'delivery_address' => '123 Customer Street, User City, CA 90210',
    ...
    [
        ...
        'delivery_address' => '456 Buyer Avenue, Customer Town, NY 10001',
    ...
    [
        ...
        'delivery_address' => '789 Order Street, Purchase City, TX 73301',
...
echo "\n🗑️ Step 5: Soft-deleting 1 item from each type (as requested)\n";
...
echo "\n✅ Final Status (What you'll see in views):\n";
echo "  📂 Active Categories: " . ProductCategory::count() . " (3 visible)\n";
echo "  📦 Active Products: " . Product::count() . " (15 visible)\n";
echo "  🏪 Active Stores: " . Store::count() . " (3 visible)\n";
echo "  🛒 Total Orders: " . Order::count() . " (3 visible)\n";

echo "\n🎉 Fresh test data created successfully!\n";
echo "✅ You can now see the data in your views\n";
echo "✅ 1 item from each type has been soft-deleted to test deletion functionality\n";
