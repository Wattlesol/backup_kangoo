...
@php
    ...
    $totalRequests = $stats['active'] + $stats['completed'];
    $pendingPayments = $activeRequests->filter(fn ($request) => !$request->payment || $request->payment->payment_status !== 'paid')->count();
    $approvedDocuments = $activeRequests->sum(fn ($request) => collect($request->sanadDocuments ?? [])->where('verification_status', 'verified')->count());
    $weeklyActivity = [42, 66, 48, 84, 58, 92, 72];
    $weekDays = $isAr ? ['س','ح','ن','ث','ر','خ','ج'] : ['S','M','T','W','T','F','S'];
...
<style>
    ...
    .quick-insights{display:grid;grid-template-columns:minmax(0,1.25fr) minmax(290px,.75fr);gap:16px;margin-top:16px}.quick-panel{border:1px solid #d8e4f2;border-radius:18px;background:#fff;box-shadow:0 7px 22px rgba(15,41,51,.04);padding:22px}.quick-panel-title{font-size:15px;font-weight:900}.quick-panel-subtitle{margin-top:3px;color:#6a7c93;font-size:12px}.quick-chart{display:grid;grid-template-columns:repeat(7,1fr);align-items:end;gap:10px;height:180px;margin-top:22px}.quick-chart-column{display:flex;height:100%;flex-direction:column;justify-content:flex-end;gap:7px;text-align:center}.quick-chart-bar{position:relative;border-radius:10px 10px 3px 3px;background:#dceaff;overflow:hidden}.quick-chart-bar span{position:absolute;inset-inline:0;bottom:0;border-radius:9px 9px 2px 2px;background:linear-gradient(0deg,#1769ff,#20c5e8)}.quick-chart-column small{color:#8a9ab0;font-weight:800}
...
<div class="quick-customer-dashboard" dir="{{ $isAr ? 'rtl' : 'ltr' }}">
    ...
    <section class="quick-insights">
        <article class="quick-panel"><div class="quick-panel-title">{{ $isAr ? 'نشاط الطلبات — آخر 7 أيام' : 'Request activity — last 7 days' }}</div><div class="quick-panel-subtitle">{{ $isAr ? 'طلبات جديدة ومراحل تم إنجازها' : 'New submissions and completed stages' }}</div><div class="quick-chart">@foreach($weeklyActivity as $index => $height)<div class="quick-chart-column"><div class="quick-chart-bar" style="height:{{ $height }}%"><span style="height:{{ max(30,$height - 18) }}%"></span></div><small>{{ $weekDays[$index] }}</small></div>@endforeach</div></article>
        <article class="quick-panel"><div class="quick-panel-title">{{ $isAr ? 'مركز الإجراءات' : 'Action center' }}</div><div class="quick-action-list">
            <a class="quick-action-item" href="{{ route('customer-portal.vault') }}"><i class="fas fa-file-alt"></i><span>{{ $isAr ? 'مستندات معلقة' : 'Pending documents' }}</span><strong>{{ $stats['pending_actions'] }}</strong></a>
            <a class="quick-action-item" href="{{ route('customer-portal.billing') }}"><i class="fas fa-credit-card"></i><span>{{ $isAr ? 'دفعات معلقة' : 'Pending payments' }}</span><strong>{{ $pendingPayments }}</strong></a>
            <a class="quick-action-item" href="{{ route('customer-portal.messages') }}"><i class="fas fa-comments"></i><span>{{ $isAr ? 'الرسائل' : 'Messages' }}</span><strong>{{ $activities->count() }}</strong></a>
            <a class="quick-action-item" href="{{ route('customer-portal.vault') }}"><i class="fas fa-shield-alt"></i><span>{{ $isAr ? 'وثائق معتمدة' : 'Approved documents' }}</span><strong>{{ $approvedDocuments }}</strong></a>
        </div></article>
    </section>
    ...
    <section class="quick-panel quick-tracker">
        ...
        @forelse($activeRequests as $request)
            ...
            <div class="quick-request-card">
                ...
                <a class="sanad-btn" href="{{ route('customer-portal.requests.show', $request->id) }}">{{ $isAr ? 'تتبع ومحادثة' : 'Track & Chat' }}</a>
            </div>
        @empty
