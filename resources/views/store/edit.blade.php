<x-master-layout>
    <div class="container-fluid">
        <div class="row">
                ...
                <div class="card card-block card-stretch">
                    <div class="card-body p-0">
                        <div class="d-flex justify-content-between align-items-center p-3 flex-wrap gap-3">
                            <h5 class="font-weight-bold">{{ $pageTitle ?? trans('messages.edit_form_title',['form' => trans('messages.store')]) }}</h5>
            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-body">
                        {{ Form::model($storedata,['method' => 'POST','route'=>['store.update', $storedata->id], 'enctype'=>'multipart/form-data', 'data-toggle'=>"validator" ,'id'=>'store'] ) }}
                        @method('PUT')
                        {{ Form::hidden('id') }}
                                    ...
                                    <div class="card-body">
                                        <div class="row">
                                            ...
                                            <div class="form-group col-md-6">
                                                ...
                                                {{ Form::select('status', ['pending' => __('messages.pending'), 'approved' => __('messages.approved'), 'rejected' => __('messages.rejected'), 'suspended' => __('messages.suspended')], old('status'), ['class' => 'form-control select2js', 'required']) }}
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div class="card-body">
                                        <div class="row">
                                            ...
                                            <div class="form-group col-md-12">
                                                ...
                                                {{ Form::select('is_active', ['1' => __('messages.active'), '0' => __('messages.inactive')], old('is_active'), ['class' => 'form-control select2js']) }}
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="col-md-4">
                                ...
                                <div class="card mt-3">
                                    ...
                                    <div class="card-body">
                                        ...
                                        @if($storedata->approved_at)
                                            <div class="form-group">
                                                <label class="form-control-label">{{ __('messages.approved_at') }}</label>
                                                <p class="form-control-static">{{ $storedata->approved_at->format('Y-m-d H:i:s') }}</p>
                                            </div>
                                        @endif
                                        
                                        @if($storedata->approvedBy)
                                            <div class="form-group">
                                                <label class="form-control-label">{{ __('messages.approved_by') }}</label>
                                                <p class="form-control-static">{{ $storedata->approvedBy->display_name }}</p>
                                            </div>
                                        @endif
                                    </div>
                                </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            // Load existing states and cities if editing
            @if($storedata->country_id)
                loadStates({{ $storedata->country_id }}, {{ $storedata->state_id ?? 'null' }});
            @endif
            
            @if($storedata->state_id)
                loadCities({{ $storedata->state_id }}, {{ $storedata->city_id ?? 'null' }});
            @endif
            ...
            function loadStates(countryId, selectedStateId = null) {
                $.get('/get-states/' + countryId, function(data) {
                    $.each(data, function(key, value) {
                        var selected = selectedStateId == value.id ? 'selected' : '';
            ...
            function loadCities(stateId, selectedCityId = null) {
                $.get('/get-cities/' + stateId, function(data) {
                    $.each(data, function(key, value) {
                        var selected = selectedCityId == value.id ? 'selected' : '';
