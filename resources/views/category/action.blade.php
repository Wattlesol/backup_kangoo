...
@endphp
{{ Form::open(['route' => ['category.destroy', $data->id], 'method' => 'delete', 'data--submit'=>'category'.$data->id, 'style' => 'margin:0;display:inline;']) }}
<div class="quick-table-actions" style="display: flex; align-items: center; justify-content: flex-end; gap: 6px;">
    @if(auth()->user()->can('category edit'))
        <a href="{{ route('category.create', ['id' => $data->id]) }}" 
           class="quick-action-btn quick-action-btn-edit" 
           title="{{ __('messages.update_form_title',['form'=> __('messages.category')]) }}"
    ...
    @if(!$data->trashed())
        ...
        <a class="quick-action-btn quick-action-btn-delete delete-category" 
           href="{{ route('category.destroy', $data->id) }}" 
           data--submit="category{{$data->id}}" 
           data--ajax="true"
