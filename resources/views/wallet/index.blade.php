@php
    ...
    $pageTitle = $pageTitle ?? ($isAr ? 'المحافظ المالية' : 'Wallet Directory');
@endphp
...
<x-master-layout>
    ...
    <div class="quick-wallet-page" dir="{{ $isAr ? 'rtl' : 'ltr' }}">

        <!-- 1. Hero Banner Card -->
        <div class="quick-admin-hero">
            <div class="quick-admin-hero-content">
                <div class="quick-admin-hero-eyebrow">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:14px;height:14px;"><path d="M20 7H4a2 2 0 0 0-2 2v10a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V9a2 2 0 0 0-2-2z"/><path d="M16 12h4"/></svg>
                    <span>{{ $isAr ? 'المعاملات وإدارة الأرصدة' : 'Digital Wallets & Balance Management' }}</span>
                </div>
                <h1>{{ $pageTitle }}</h1>
                <p>{{ $isAr ? 'متابعة أرصدة محافظ المستخدمين والشركاء، شحن الأرصدة، وتدقيق العمليات المالية والحركات المجمعة.' : 'Manage user and partner digital wallets, balance adjustments, and audit transaction records.' }}</p>
            </div>

            <div class="quick-admin-hero-actions">
                <a href="{{ route('wallet.create') }}" class="quick-admin-hero-btn quick-admin-hero-btn-primary">
                    ...
                    <span>{{ $isAr ? 'إضافة محفظة جديدة' : 'Add Wallet' }}</span>
                </a>
                <a href="{{ route('payment.index') }}" class="quick-admin-hero-btn quick-admin-hero-btn-secondary">
        ...
        <div class="quick-card">
            ...
            <div class="quick-card-header">
                <div>
                    <h3 class="quick-card-title">{{ $isAr ? 'سجل المحافظ والأرصدة' : 'Official Wallets Directory' }}</h3>
                    <div class="quick-card-sub">{{ $isAr ? 'عرض وتعديل وتصفية محافظ الشركاء والمستخدمين ومتابعة الأرصدة الحالية' : 'View, edit, filter, and perform bulk operations on wallets' }}</div>
                </div>

                <div class="quick-provider-toolbar-actions">
                    <!-- Status Filter Pills -->
                    <div class="quick-filter-pills" role="tablist">
                        <button type="button" class="{{ $filter['status'] === '' || $filter['status'] === null ? 'active' : '' }}" onclick="filterWalletStatus('', this)">{{ $isAr ? 'الكل' : 'All' }}</button>
                        <button type="button" class="{{ (string) $filter['status'] === '1' ? 'active' : '' }}" onclick="filterWalletStatus('1', this)">{{ $isAr ? 'النشطة' : 'Active' }}</button>
                        <button type="button" class="{{ (string) $filter['status'] === '0' ? 'active' : '' }}" onclick="filterWalletStatus('0', this)">{{ $isAr ? 'غير النشطة' : 'Inactive' }}</button>
                    </div>
                    ...
                    <div class="quick-search-box">
                        ...
                        <input type="text" class="quick-search-input dt-search" placeholder="{{ $isAr ? 'بحث في المحافظ...' : 'Search wallets...' }}" aria-label="Search wallets">
                    </div>
                </div>
            </div>
            ...
            <div class="quick-bulk-bar">
                <form action="{{ route('wallet.bulk-action') }}" id="quick-action-form" class="quick-bulk-form form-disabled">
                    @csrf
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            window.renderedDataTable = $('#datatable').DataTable({
                ...
                language: {
                    ...
                    info: "{{ $isAr ? 'عرض _START_ إلى _END_ من أصل _TOTAL_ محفظة' : 'Showing _START_ to _END_ of _TOTAL_ entries' }}",
        ...
        function filterWalletStatus(status, btn) {
