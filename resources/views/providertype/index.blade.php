@php
    ...
    $pageTitle = $isAr ? 'دليل أنواع الشركاء' : 'Partner Types Directory';
@endphp
...
<x-master-layout>
    ...
    <div class="quick-providertype-page" dir="{{ $isAr ? 'rtl' : 'ltr' }}">

        <!-- 1. Hero Banner Card -->
        <div class="quick-admin-hero">
            <div class="quick-admin-hero-content">
                <div class="quick-admin-hero-eyebrow">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:14px;height:14px;"><path d="M20 7h-9"/><path d="M14 17H5"/><circle cx="17" cy="17" r="3"/><circle cx="7" cy="7" r="3"/></svg>
                    <span>{{ $isAr ? 'تصنيفات وهيكلية الشركاء' : 'Partner Classification & Commission Structure' }}</span>
                </div>
                <h1>{{ $pageTitle }}</h1>
                <p>{{ $isAr ? 'إدارة فئات الشركاء، وتحديد نسب العمولة الافتراضية، وضبط حالات التفعيل والنشاط.' : 'Manage partner categories, default commission rates, contract terms, and operational statuses.' }}</p>
            </div>

            <div class="quick-admin-hero-actions">
                @if($auth_user->can('providertype add'))
                    <a href="{{ route('providertype.create') }}" class="quick-admin-hero-btn quick-admin-hero-btn-primary">
                        ...
                        <span>{{ $isAr ? 'إضافة نوع شريك' : 'Add Partner Type' }}</span>
                    </a>
                @endif
                <a href="{{ route('provider.index') }}" class="quick-admin-hero-btn quick-admin-hero-btn-secondary">
                    ...
                    <span>{{ $isAr ? 'دليل الشركاء' : 'Partner Directory' }}</span>
                </a>
            </div>
        </div>
        ...
        <div class="quick-card">
            ...
            <div class="quick-card-header">
                <div>
                    <h3 class="quick-card-title">{{ $isAr ? 'سجل أنواع وتصنيفات الشركاء' : 'Official Partner Types Directory' }}</h3>
                    <div class="quick-card-sub">{{ $isAr ? 'عرض وتعديل وتصفية فئات الشركاء ونسب العمولة' : 'View, edit, filter, and perform bulk operations on partner types' }}</div>
                </div>

                <div class="quick-provider-toolbar-actions">
                    <!-- Status Filter Pills -->
                    <div class="quick-filter-pills" role="tablist">
                        <button type="button" class="{{ $filter['status'] === '' || $filter['status'] === null ? 'active' : '' }}" onclick="filterTypeStatus('', this)">{{ $isAr ? 'الكل' : 'All' }}</button>
                        <button type="button" class="{{ (string) $filter['status'] === '1' ? 'active' : '' }}" onclick="filterTypeStatus('1', this)">{{ $isAr ? 'النشطة' : 'Active' }}</button>
                        <button type="button" class="{{ (string) $filter['status'] === '0' ? 'active' : '' }}" onclick="filterTypeStatus('0', this)">{{ $isAr ? 'غير النشطة' : 'Inactive' }}</button>
                    </div>
                    ...
                    <div class="quick-search-box">
                        ...
                        <input type="text" class="quick-search-input dt-search" placeholder="{{ $isAr ? 'بحث في أنواع الشركاء...' : 'Search partner types...' }}" aria-label="Search partner types">
                    </div>
                </div>
            </div>
            ...
            <div class="quick-bulk-bar">
                <form action="{{ route('providertype.bulk-action') }}" id="quick-action-form" class="quick-bulk-form form-disabled">
                    @csrf
    ...
    <script>
        ...
        function filterTypeStatus(status, btn) {
