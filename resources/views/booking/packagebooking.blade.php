                        ...
                        @foreach ($package_service_booking as $package)
                            <tr>
                                ...
                                <td>
                                    ...
                                    <form action="{{route('servicepackage.ChangeData',$package->id)}}" method="POST">
                                        @csrf
                                    ...
                                    <br>
                                    <a href="{{route('servicepackage.change_status',[$package->id,\App\Enums\BookingEnums::approved])}}" class="btn btn-primary mt-2">Confirm</a>
                                    @endif

                                    @if($package->status == \App\Enums\BookingEnums::approved)
                                            <form action="{{route('servicepackage.AssignHandyman',$package->id)}}" method="POST">
                                                @csrf
                                                <div class="form-group">
                                        ...
                                        <br>
                                        <a href="{{route('servicepackage.change_status', [$package->id, \App\Enums\BookingEnums::canceled])}}" class="btn btn-danger mt-2">Cancel</a>
                                    @endif
                                        @if($package->status == \App\Enums\BookingEnums::handyman_assign)

                                            <a href="{{route('servicepackage.change_status', [$package->id, \App\Enums\BookingEnums::finished])}}" class="btn btn-danger mt-2">Finished</a>
                                        @endif
                                </td>
                            </tr>
                        @endforeach
