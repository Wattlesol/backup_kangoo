                                ...
                                <td style="text-align: {{ $isAr ? 'left' : 'right' }};">
                                    <div style="display: flex; flex-direction: column; gap: 8px; align-items: flex-end;">
                                        @if(in_array($package->status, [\App\Enums\BookingEnums::WaitingForApproval, \App\Enums\BookingEnums::reschedule]))
                                            <form action="{{ route('proiver_booking_servicepackage.ChangeData', $package->id) }}" method="POST" style="display:flex;gap:6px;align-items:center;flex-wrap:wrap;">
                                                @csrf
                                            ...
                                            <a href="{{ route('proiver_booking_servicepackage.change_status', [$package->id, \App\Enums\BookingEnums::approved]) }}" class="quick-admin-hero-btn quick-admin-hero-btn-primary" style="min-height:34px;padding:4px 12px;font-size:12px;">
                                                {{ $isAr ? 'تأكيد الحجز' : 'Confirm' }}
                                            </a>
                                        @endif

                                        @if($package->status == \App\Enums\BookingEnums::approved)
                                            <form action="{{ route('proiver_booking_servicepackage.AssignHandyman', $package->id) }}" method="POST" style="display:flex;gap:6px;align-items:center;flex-wrap:wrap;">
                                                @csrf
                                            ...
                                            <a href="{{ route('proiver_booking_servicepackage.change_status', [$package->id, \App\Enums\BookingEnums::canceled]) }}" class="quick-admin-hero-btn quick-admin-hero-btn-secondary" style="min-height:34px;padding:4px 12px;font-size:12px;color:#ef4444 !important;">
                                                {{ $isAr ? 'إلغاء' : 'Cancel' }}
                                            </a>
                                        @endif

                                        @if($package->status == \App\Enums\BookingEnums::handyman_assign)
                                            <a href="{{ route('proiver_booking_servicepackage.change_status', [$package->id, \App\Enums\BookingEnums::finished]) }}" class="quick-admin-hero-btn quick-admin-hero-btn-primary" style="min-height:34px;padding:4px 12px;font-size:12px;background:#10b981;">
