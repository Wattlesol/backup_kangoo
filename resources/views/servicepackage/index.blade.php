@php
    ...
    $pageTitle = $isAr ? 'باقات الخدمات المجمعة' : 'Service Bundles & Packages';
    $summary = $packageSummary ?? [
        'total' => AppModelsServicePackage::count(),
        'active' => AppModelsServicePackage::where('status', 1)->count(),
        'inactive' => AppModelsServicePackage::where('status', 0)->count(),
        'featured' => AppModelsServicePackage::where('is_featured', 1)->count(),
...
<x-master-layout>
    <div class="quick-package-page" dir="{{ $isAr ? 'rtl' : 'ltr' }}">
        <!-- 1. Hero Banner Card -->
        <div class="quick-admin-hero">
            <div class="quick-admin-hero-content">
                <div class="quick-admin-hero-eyebrow">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:14px;height:14px;"><path d="m7.5 4.27 9 5.15"/><path d="M21 8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16Z"/><path d="m3.3 7 8.7 5 8.7-5"/><path d="M12 22V12"/></svg>
                    <span>{{ $isAr ? 'العروض والباقات الخدمية المجمعة' : 'Bundled Service Offerings' }}</span>
                </div>
                <h1>{{ $isAr ? 'باقات الخدمات المجمعة' : 'Service Bundles & Packages' }}</h1>
                <p>{{ $isAr ? 'تجميع وتضمين عدة خدمات حكومية ضمن باقة واحدة بأسعار مميزة وتحديد معايير وشروط الاستخدام.' : 'Bundle multiple government services into unified packages with discounted pricing and clear terms.' }}</p>
            </div>

            <div class="quick-admin-hero-actions">
                @if(isset($auth_user) && $auth_user->can('servicepackage add'))
                    <a href="{{ route('servicepackage.create') }}" class="quick-admin-hero-btn quick-admin-hero-btn-primary">
                        ...
                        <span>{{ $isAr ? 'إنشاء باقة جديدة' : 'Create Bundle' }}</span>
                    </a>
                @endif
        ...
        <div class="quick-kpi-grid">
            ...
            <div class="quick-kpi-card">
                <div class="quick-kpi-header">
                    <span>{{ $isAr ? 'إجمالي الباقات' : 'Total Bundles' }}</span>
            ...
            <div class="quick-kpi-card">
                <div class="quick-kpi-header">
                    <span>{{ $isAr ? 'الباقات النشطة' : 'Active Bundles' }}</span>
                ...
                <div class="quick-kpi-sub">
                    ...
                    <span>{{ $isAr ? 'متاحة للطلب للبوابة' : 'live in portal' }}</span>
                </div>
            </div>
            ...
            <div class="quick-kpi-card">
                <div class="quick-kpi-header">
                    <span>{{ $isAr ? 'الباقات المميزة' : 'Featured Packages' }}</span>
                ...
                <div class="quick-kpi-sub">
                    ...
                    <span>{{ $isAr ? 'مروجة في الواجهة' : 'promoted bundles' }}</span>
                </div>
            </div>

            <!-- Metric 4: Services Included -->
            <div class="quick-kpi-card">
                <div class="quick-kpi-header">
                    <span>{{ $isAr ? 'الخدمات المتاحة' : 'Eligible Services' }}</span>
                ...
                <div class="quick-kpi-sub">
                    ...
                    <span>{{ $isAr ? 'خدمة حكومية قابلة للدمج' : 'services in catalog' }}</span>
                </div>
            </div>
        </div>

        <!-- 3. Package Data Table Card -->
        <div class="quick-card quick-package-table-card">
            <!-- Toolbar: Header & Search & Bulk Actions -->
            <div class="quick-card-header">
                <div>
                    <h3 class="quick-card-title">{{ $isAr ? 'قائمة باقات الخدمات الرسمية' : 'Official Service Bundles Directory' }}</h3>
                    <div class="quick-card-sub">{{ $isAr ? 'عرض وتعديل وتصفية باقات الخدمات وإدارة حالات النشاط' : 'View, edit, filter, and manage service packages and active statuses' }}</div>
                </div>

                <div class="quick-category-toolbar-actions">
                    <!-- Status Filter Pills -->
                    <div class="quick-filter-pills" role="tablist">
                        <button type="button" class="active" onclick="filterPackageStatus('', this)">{{ $isAr ? 'الكل' : 'All' }}</button>
                        <button type="button" onclick="filterPackageStatus('1', this)">{{ $isAr ? 'النشطة' : 'Active' }}</button>
                        <button type="button" onclick="filterPackageStatus('0', this)">{{ $isAr ? 'غير النشطة' : 'Inactive' }}</button>
                    </div>
                    ...
                    <div class="quick-search-box">
                        ...
                        <input type="text" class="quick-search-input dt-search" placeholder="{{ $isAr ? 'بحث في الباقات...' : 'Search bundles...' }}" aria-label="Search bundles">
                    </div>
                </div>
            </div>
            ...
            <div class="quick-bulk-bar">
                <form action="{{ route('servicepackage.bulk-action') }}" id="quick-action-form" class="quick-bulk-form form-disabled">
                    @csrf
    ...
    <style>
        ...
        .quick-package-table-card .quick-table th {
            padding: 14px 16px;
        }

        .quick-package-table-card .quick-table td {
            padding: 14px 16px;
        }
    ...
    <script>
            ...
            window.renderedDataTable = $('#datatable').DataTable({
                ...
                ajax: {
                    ...
                    data: function (d) {
                        ...
                        d.filter = {
                            column_status: currentPackageStatus
                        };
                    }
                },
                columns: [
                ...
                language: {
                    ...
                    emptyTable: "{{ $isAr ? 'لا توجد باقات متاحة.' : 'No service bundles found.' }}",
                    info: "{{ $isAr ? 'عرض _START_ إلى _END_ من أصل _TOTAL_ باقة' : 'Showing _START_ to _END_ of _TOTAL_ entries' }}",
        ...
        function filterPackageStatus(status, btn) {
