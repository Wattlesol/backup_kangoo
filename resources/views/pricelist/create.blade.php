...
<x-master-layout>
    <div class="container-fluid">
        ...
        <form class="mt-4" action="{{route('pricelist.store')}}" method="post" enctype="multipart/form-data">
            {{csrf_field()}}
            <div class="row">
                <div class="col-md-12">
                                    ...
                                    <td >
                                            <select name="city_id[]" class="form-control" >
                                                @foreach($city as $cities)
                                                    <option value=" {{ $cities->id }}" @if($cities->id == $cities2->id)selected @endif > {{ $cities->name }}</option>
                                                @endforeach
                                            </select>
                                    </td>
                                    <td >
