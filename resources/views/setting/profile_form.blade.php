...
@php
    $regestion = \App\Models\CityRegion::pluck('name','id');
@endphp
                    <div class="form-group col-md-6">
                        {{ Form::label('country_id', __('messages.select_name',[ 'select' => "Area" ]),['class'=>'form-control-label'],false) }}
                        <br />
                        {{ Form::select('country_id', $regestion, [], [
								'class' => 'select2js form-group country',
                                'id'=>'country_id',
								'data-placeholder' => __('messages.select_name',[ 'select' => __('messages.country') ]),
							]) }}
...
$(document).ready(function() {
    ...
    function city_id_f(state, city = "") {
        var city_route = "{{ route('ajax-list', [ 'type' => 'sub_araeas' ,'state_id' =>'']) }}" + state;
        city_route = city_route.replace('amp;', '');
