...
assert_contains "$admin_service_list" "Quick Service Catalog" "admin service list"
...
assert_contains "$admin_service_create" "Quick Service Master Data" "admin service form"
...
assert_contains "$admin_service_create" "Quick Service Fee" "admin service form"
...
pass "admin service catalog and Quick master data frontend markers"

partner_cookies="$(login "$PARTNER_EMAIL" partner)"
curl -sS -b "$partner_cookies" -c "$partner_cookies" -L "${BASE_WEB_URL}/lang/ar" >/dev/null
partner_service_list="$(fetch_text "$partner_cookies" partner /service)"

assert_contains "$partner_service_list" "دليل خدمات كويك" "partner service list"
assert_contains "$partner_service_list" "جاهزية الدليل" "partner service list"
assert_contains "$partner_service_list" "الخدمات النشطة" "partner service list"
