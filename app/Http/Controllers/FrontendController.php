...
class FrontendController extends Controller
{
    ...
    public function servicePackage_List(Request $request){
        $servicePack = ServicePackage::where('status',1)->get();
        return view('landing-page.servicepackages', compact('servicePack'));
    }
    ...
    public function servicepackageDetail( $id){
        $settings = Setting::whereIn('type', ['general-setting', 'social-media', 'site-setup'])
        ...
        $serviceData  = ServicePackage::findOrFail($id);
        $pakcages = PriceCity::where('price_list_id',$serviceData->pricelist_id)->pluck('city_id')->toArray();
        $region = Districts::whereIn('region_id',$pakcages)->get();
        return view('landing-page.ServicePackageDetail', compact('serviceData','region','generalsetting'));
    }

    public function checkPrice(Request  $request)
    {
        $service = ServicePackage::findOrFail($request->service_id);
        $Districts = Districts::findOrFail($request->city_id);
        $priceList = PriceCity::where('price_list_id',$service->pricelist_id)->where('city_id',$Districts->region_id)->first();
        if ($priceList){
            return $priceList->price;
        }else{
            return "not Available for this area ";
        }
    }
    public function buy_package_data(Request  $request)
    {
        $service = ServicePackage::findOrFail($request->service_id);
        $Districts = Districts::findOrFail($request->city_id);
        $priceList = PriceCity::where('price_list_id',$service->pricelist_id)->where('city_id',$Districts->region_id)->first();
        if (!$priceList){
            return redirect()->back()->with('error','not found price');
        }
        $Districts = Districts::findOrFail($request->city_id);

        $region_data = Region::where('id',$Districts->region_id)->first();
        $Subscription = ServicePacakgeSubscription::create([
            'user_id' => auth()->user()->id,
            'package_id'=>$service->id,
            'start_at'=>Carbon::now()->toDateString(),
            'end_at'=>Carbon::now()->addDays($service->duration)->toDateString(),
            'price'=>$priceList->price,
            'package_type'=>$service->package_type,
            'car_number'=>$service->car_number,
            'region_id'=>$Districts->region_id,
            'city_id'=>$region_data->city_id
        ]);

        if ($service->package_service_data){

            foreach ($service->package_service_data as $serviceData){
                ServicePacakgeServiceSubscription::create([
                    'service_id'=>$serviceData->service_id,
                    'service_type_data'=>$serviceData->service_type_data,
                    'count'=>$serviceData->count,
                    'usage_times'=>$serviceData->usage_times,
                    'duration_of_use'=>$serviceData->duration_of_use,
    ...
    public function submitForm(Request $request)
    {
        ...
        // Send email or handle form submission
